...
class ExpandSequenceFunction(torch.autograd.Function):
    ...
    def forward(ctx, X_seq):
        ...
        expanded_probs = torch.zeros((new_seq_len, new_vocab_size), dtype=X_seq.dtype, device=X_seq.device)
...
class PredictorAPEX_Probs:
    ...
    def predict(self, probs, return_x=False):
        ...
        for ensemble_id in range(len(self.APEX_models)):
            ...
            data_len = probs.shape[0]  # batch size
            ...
            for i in range(int(math.ceil(data_len / float(self.batch_size)))):
                ...
                AMP_pred_batch = AMP_model.forward_for_probs(expanded_probs)  # make predictions
                AMP_pred_batch = 10 ** (
                    6 - AMP_pred_batch
                )  # transform back to MICs; When training the APEX models, MICs were transformed by: -np.log10(MICs/float(1000000))
