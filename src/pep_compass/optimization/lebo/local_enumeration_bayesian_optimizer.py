...
from pep_compass.optimization.black_box.black_box import BlackBox
...
class LocalEnumerationBayesianOptimizer(AbstractOptimizer):
    ...
    def _turbo_filter(self):
        ...
        for distance in range(self.trust_region_distance, 25):
            filtered = self.not_scored_peptides_bktree.find(self.the_best_peptide, distance)
        ...
        return [peptide for _, peptide in filtered if peptide not in self.scored_peptides]
    ...
    def _bayesian_optimization(self, max_scorer_calls) -> str:
        ...
        with self.timer("extract train features"):
            ...
            train_X = torch.tensor(train_features, dtype=torch.float64, requires_grad=False).to(self.device)
            train_Y = torch.tensor(train_scores, dtype=torch.float64, requires_grad=False).reshape(-1, 1).to(self.device)
        ...
        with self.timer("extract test features"):
            ...
            test_X = torch.tensor(test_peptides_features, dtype=torch.float64, requires_grad=False).cpu().unsqueeze(1)
        ...
        with self.timer("compute logEI"):
            ...
            with torch.no_grad():
                ...
                for i in range(0, len(test_X), self.acquisition_batch_size):
                    ...
                    logger.debug(f"Computed logEI for batch {i // self.acquisition_batch_size + 1}")
                    # del batch
                test_logEI = torch.cat(test_logEI)
            
            logger.info(f"Acquisition function computed for {len(test_peptides)} test peptides")
        ...
        if self.turbo_success_counter >= self.turbo_success_tolerance:
            ...
            logger.info(f"Trust region distance increased to {self.trust_region_distance}")
            self.turbo_success_counter = 0
        ...
        if self.turbo_failure_counter >= self.turbo_failure_tolerance:
            ...
            logger.info(f"Trust region distance decreased to {self.trust_region_distance}")
            self.turbo_failure_counter = 0
    ...
    def _extract_features(self, peptides) -> np.ndarray:
        logger.info(f"Peptides in cache {len(self.peptide_features)} before extraction.")
    ...
    def optimize(
        ...
        while self.black_box_calls < evaluation_budget:
            ...
            with self.timer("Local Search"):
                local_candidate_set = self.local_enumerator.local_enumeration(current_center_peptide)
                

            with self.timer("Update not scored peptides"):
                peptides_to_add = set(local_candidate_set) - set(self.scored_peptides.keys())
            ...
            logger.info(
                f"Scorer calls: {self.black_box_calls} / {evaluation_budget}"
            )
            logger.info(f"Number of not scored peptides: {len(self.not_scored_peptides_set)}")