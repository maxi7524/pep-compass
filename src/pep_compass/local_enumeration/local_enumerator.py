...
from pep_compass.local_enumeration.mutation.utils import (
...
class PotentialFilteredMutationLocalEnumerator(LocalEnumerator):
    """Local enumerator that scores mutations with a MutationPotential and
    filters candidates using nucleus-style (top-p) cumulative probability
    thresholding."""
    ...
    def _top_p_filter(
        ...
        """Return indices (into the already-descending-sorted log_potentials)
        that fall within the nucleus of cumulative probability mass ``top_p``."""
        if top_p >= 1.0:
        ...
        mask = np.empty(len(cumsum), dtype=np.bool_)
        mask[0] = True
    ...
    def local_enumeration(self, center_peptide: str, **kwargs) -> set[str]:
        ...
        kept_idx = self._top_p_filter(
            dist.log_potentials, self.top_p, self.temperature
        )
        ...
        logger.info(
            f"PotentialFiltered: {len(dist.sequences)} candidates -> "
            f"{len(kept_idx)} after top_p={self.top_p} (T={self.temperature}) -> "
            f"{len(neighbor_peptides)} after Levenshtein filter"
        )
