...
from pep_compass.local_enumeration.sampling.sampling_walker import SamplingWalker
...
class LocalEnumerator(ABC):
    ...
    def local_enumeration(self, center_sequence, **kwargs) -> set[str]:
        """
...
class EuclideanWalkerLocalSearcher(LocalEnumerator):
    ...
    def local_enumeration(
        self, center_peptide, batch_size=1000, **kwargs
    ) -> Neigborhood:
...
class WalkerMutationLocalSearcherWithAnnealing(LocalEnumerator):
    ...
    # @mem_profile
    def local_enumeration(
        self, center_peptide, current_step=None, max_step=None, **kwargs
    ) -> Neigborhood:
        ...
        if current_step is not None and max_step is not None:
            ...
            self.walker_time = (
                self.min_walker_time
                + (self.max_walker_time - self.min_walker_time) * cosine
            )
        ...
        for trajectory_iter in range(self.walker_trajectories_number):
            ...
            while self.walker.time < self.walker_time:
                ...
                mutated_peptides = self.mutation_generator.mutate(
                    step_peptide, self.walker.current_step
                )
...
class NormalSamplingLocalSearcher(LocalEnumerator):
    ...
    def local_enumeration(
        self, center_peptide, batch_size=5000, **kwargs
    ) -> set[str]:
...
class MutationLocalSearcher(LocalEnumerator):
    ...
    def local_enumeration(self, center_peptide, **kwargs) -> set[str]:
        ...
        neighbor_peptides = {
            peptide
            for peptide in mutated_peptides
            if Levenshtein.distance(peptide, center_peptide) <= self.max_neighbour_levenstein
        }
