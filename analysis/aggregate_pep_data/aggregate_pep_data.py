...
def load_single_input(
    ...
    df[merge_cfg.source_col] = input_path.name

    df = df.rename(columns=input_cfg.rename_map)

    if merge_cfg.min_length is not None or merge_cfg.max_length is not None:
        seq_lengths = df[merge_cfg.seq_col].astype(str).str.len()
        if merge_cfg.min_length is not None:
            df = df[seq_lengths >= merge_cfg.min_length]
        if merge_cfg.max_length is not None:
            df = df[seq_lengths <= merge_cfg.max_length]

    for key, value in input_cfg.fixed_columns.items():
        df[key] = value

    if merge_cfg.keep_columns:
        df = df[merge_cfg.keep_columns]
