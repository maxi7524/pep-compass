"""Run MOVE latent-displacement-filter LEBO against the APEX oracle."""
...
from pep_compass.local_enumeration.mutation.mutation_filters import MoveFilter
    ...
    for name, (sequence, repetitions) in PROTEINS.items():
        for repetition in range(repetitions):
            ...
            observer.initialize_observer(
                ...
                {
                    "experiment_id": f"move_{name}_{seed}_{datetime.now():%Y%m%d_%H%M%S}",
