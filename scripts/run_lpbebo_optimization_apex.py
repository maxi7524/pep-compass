"""Run decoder-log-probability LPBEBO against the APEX oracle."""
...
from pep_compass.local_enumeration.mutation.mutation_filters import LpbeboFilter
...
def main() -> None:
    ...
    local_enumerator = FilteredMutationLocalEnumerator(
        encoder_decoder=encoder_decoder,
        mutation_generator=mutation_enumerator,
        candidate_filter=LpbeboFilter(
            encoder_decoder,
            top_p=TOP_P,
    ...
    for name, (sequence, repetitions) in PROTEINS.items():
        for repetition in range(repetitions):
            ...
            observer.initialize_observer(
                ...
                {
                    "experiment_id": f"lpbebo_{name}_{seed}_{datetime.now():%Y%m%d_%H%M%S}",
