"""Run LAMS hard-similarity-filter LEBO against the APEX oracle."""
...
from pep_compass.local_enumeration.mutation.mutation_filters import LamsFilter
    ...
    for name, (sequence, repetitions) in PROTEINS.items():
        for repetition in range(repetitions):
            ...
            observer.initialize_observer(
                ...
                {
                    "experiment_id": f"lams_{name}_{seed}_{datetime.now():%Y%m%d_%H%M%S}",
