"""Combine CSV trajectories produced by the configured LE-BO runner."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("lebo_results.csv"),
        help="Combined CSV path.",
    )
    return parser.parse_args()


def _read_config(result_file: Path, root: Path) -> tuple[str, str]:
    for parent in result_file.parents:
        config_path = parent / "resolved_config.json"
        if config_path.exists():
            with config_path.open(encoding="utf-8") as config_file:
                config = json.load(config_file)
            return parent.relative_to(root).as_posix(), config["method"]
        if parent == root:
            break
...
def main() -> None:
    ...
    result_files = sorted(
        ...
        if path.name != "grid_manifest.csv" and path.resolve() != output
    )
    fieldnames = [
        "grid_id",
        "method",
    ...
    with output.open("w", encoding="utf-8", newline="") as output_file:
        ...
        for result_file in result_files:
            grid_id, method = _read_config(result_file, root)
