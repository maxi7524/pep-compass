"""Run random-proposal or random-selection LEBO controls against APEX."""
...
# Set to `walker` for random proposals or `mutang_random` for random selection
# from the geometry-derived MUTANG proposal set.
MODE = "walker"
DEVICE = "cuda:0"
OUTPUT_PATH = "./results/random_lebo"
EVALUATION_BUDGET = 1400
SELECTION_FRACTION = 0.6
MAXIMUM_POSITIONS = 5
RESIDUES_PER_POSITION = 4
...
def main() -> None:
    ...
    local_enumerator = SamplingFilteredMutationLocalEnumerator(
        ...
        candidate_filter=RandomLeBoFilter(
            mode=MODE,
            selection_fraction=SELECTION_FRACTION,
            maximum_positions=MAXIMUM_POSITIONS,
            residues_per_position=RESIDUES_PER_POSITION,
    ...
    for name, (sequence, repetitions) in PROTEINS.items():
        for repetition in range(repetitions):
            ...
            observer.initialize_observer(
                ...
                {
                    "experiment_id": f"random_lebo_{MODE}_{name}_{seed}_{datetime.now():%Y%m%d_%H%M%S}",
                    "experiment_path": f"{OUTPUT_PATH}_{MODE}",
                },
                seed,
                encoder_decoder=encoder_decoder,
            )
